...
References: https://npml.notion.site/3300a2eb85b54c289957a9604e980354
"""
import numpy as np
...
def smape(y_true: list, y_pred: list) -> float:
    ...
    assert m == n
    x = [0 if y_true[i] == y_pred[i] == 0 else
         abs(2 * (y_true[i] - y_pred[i]) / (y_true[i] + y_pred[i])) for i in range(n)]
    return sum(x) / n
